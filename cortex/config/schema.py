...
class AgentConcurrencyConfig(BaseModel):
    ...
    # Ceiling on concurrent LLM HTTP requests. Leave unset (None) to let the
    # framework auto-derive it from your configured provider+model via the
    # model-power registry (cortex/llm/model_power.py) — e.g. 1 for a local
    # Ollama, 8 for Anthropic Haiku, 4 for Opus. With adaptive_llm_concurrency
    # enabled (default), AdaptiveLLMGate then self-tunes the gate between 1
    # and this ceiling using observed latency/errors. Set an explicit integer
    # only to pin a fixed value (benchmarking or hard-rate-limited APIs).
    max_parallel_llm_calls: Optional[int] = None
    # Whether the framework should self-tune `max_parallel_llm_calls` at run
    # time using AIMD-style adaptation. Default on — turn off for a fixed gate
    # (e.g. for deterministic benchmarking or hard-rate-limited APIs).
...
class AgentConfig(BaseModel):
    ...
    # When true, sub-task LLM calls receive the original user request and the
    # current session reasoning scratchpad in their system prompt so a worker
    # can reason about *why* its task exists. Adds modest tokens per sub-task
    # call; disable to minimise cost on latency/budget-sensitive deployments.
    inject_session_context: bool = True
    # "planned" — the task graph is generated by the decomposition LLM at
    # runtime (default; the classic Cortex behaviour). "static" — task_types
    # are executed as a fixed DAG in dependency order with no decomposition,
    # intent-gate, or capability-scout LLM calls. Code-node agents built with
    # cortex.builder.CortexBuilder.node() set this to "static" automatically.
...
class ReactConfig(BaseModel):
    """Tunables for the per-sub-agent ReAct (reason -> act -> observe) loop.

    The loop is always on for LLM-driven tasks -- there is no enable/disable
    flag. Every non-scripted task runs the loop, and it terminates as soon as
    the sub-agent's LLM decides the task is complete. These knobs only bound
    the loop's cost and protect against runaway iteration.
    """
    model_config = ConfigDict(extra='allow')
    # Hard safety cap. On reaching it the loop stops calling actions and forces
    # the sub-agent to emit its best final answer. Normal tasks finish well
    # before this -- it exists only to bound cost on a misbehaving loop.
    max_iterations: int = 10
    # Each tool observation is truncated to roughly this many tokens before
    # being fed back into the loop, so the running context cannot explode.
    observation_max_tokens: int = 600
    # Once the running conversation exceeds this many characters, the oldest
    # reason/act/observe steps are digested into a compact summary.
...
class TaskTypeConfig(BaseModel):
    ...
    # ReAct loop tunables (the loop itself is always on for non-scripted tasks).
...
class ValidationConfig(BaseModel):
    model_config = ConfigDict(extra='allow')
    # Master switch for the post-synthesis Validation Agent. Declared here so a
    # config built in code (CortexBuilder) has a sensible default — cortex.yaml
    # files may also set it explicitly.
    enabled: bool = True
    ...
    # Maximum number of remediation passes when a response scores between the
    # critical and pass thresholds. Each pass sees the prior attempt's response
    # and the findings it still failed on, so it can correct without repeating
    # mistakes. 1 = legacy single-shot behaviour.
...
class AppControlConfig(BaseModel):
    """Configuration for cross-platform application launch and control.

    Primary path: query the app's scripting dictionary (macOS sdef, Windows
    UI Automation / COM) and inject it as context so the LLM generates precise
    actions against the real API.

    Fallback path: when no scripting dictionary is found, enter a screenshot
    vision loop — take a screenshot, ask a vision-capable LLM for the next
    action, execute, repeat up to max_vision_steps times.

    HITL is always required — every mutating action prompts for user approval.
    """
    model_config = ConfigDict(extra='allow')
    enabled: bool = False
    hitl_enabled: bool = True       # cannot be False — enforced in framework init
    timeout_seconds: int = 30
    # Scripting dictionary
    sdef_max_chars: int = 8000      # cap on scripting dict injected into the prompt
    # Vision loop fallback
    max_vision_steps: int = 10      # max screenshot→action iterations
    vision_provider: str = "default"  # LLM provider key that supports vision/images


class PlaywrightMCPConfig(BaseModel):
    """Configuration for the built-in Playwright MCP server.

    When enabled, the framework starts @playwright/mcp as an internal stdio
    MCP server at startup. It is NOT exposed in tool_servers (users don't
    configure it); the agent receives a 'browser' capability automatically.
    ...
    # When set, Playwright persists/loads cookies and localStorage to this file
    # so login sessions survive across tasks. Default lives under the storage
    # base so each user gets an isolated session jar.
    storage_state_path: Optional[str] = None
    # User-data-dir for persistent context (more aggressive than storage_state —
    # includes service workers, IndexedDB, extensions). Mutually exclusive with
    # storage_state_path; if both set, user_data_dir wins.
    user_data_dir: Optional[str] = None
    # Viewport size for the browser window. Tuple of (width, height).
